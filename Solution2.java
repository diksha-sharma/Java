...
Each String will have a maximum of 1010 alphabetic characters, and each integer will be
in the inclusive range from 00 to 999999.
Output Format
...
public class Solution2
{
    public static void main(String[] args)
    {
        ...
        for(int i=0;i<3;i++)
        {
            String s1=sc.next();
            int x=sc.nextInt();
            int iLengthString = s1.length();
            int iLengthNumber = String.valueOf(x).length();
            for(int j=iLengthString; j< 15; j++)
            {
                s1 = s1 + " ";
            }
            String x1 = String.valueOf(x);
            for(int j=iLengthNumber; j< 3; j++)
            {
                x1 = "0" + x1;
            }
